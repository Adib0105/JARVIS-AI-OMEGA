...
class V75SelfCodingTests(unittest.TestCase):
    ...
    def _proposal(self, engine):
        proposal = engine.proposal_from_gap({
            'id': 'GAP-CODE',
            'capability': 'Coding',
            'title': 'Change greeting to new',
            'description': 'greet should return new',
            'recommended_action': 'Update implementation and regression test.',
            'evidence': ['expected=new'],
            'severity': 'MEDIUM',
        })
        return engine.prepare_sandbox(proposal.id)
    ...
    def test_generation_failure_is_repaired_in_sandbox(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            proposal = self._proposal(development)
            calls = []

            def reasoner(system, user):
                calls.append((system, user))
                if len(calls) == 1:
                    # Implementation changes but test intentionally remains old: regression must fail.
                    return '{"files":{"hello.py":"def greet():\\n    return \'new\'\\n","tests/test_hello.py":"import unittest\\nfrom hello import greet\\nclass T(unittest.TestCase):\\n    def test_greet(self):\\n        self.assertEqual(greet(), \'old\')\\n"}}'
                return '{"files":{"hello.py":"def greet():\\n    return \'new\'\\n","tests/test_hello.py":"import unittest\\nfrom hello import greet\\nclass T(unittest.TestCase):\\n    def test_greet(self):\\n        self.assertEqual(greet(), \'new\')\\n"}}'

            coding = SelfCodingEngine(development, reasoner)
            result = coding.run(proposal.id)
            self.assertEqual(result.status, ProposalStatus.AWAITING_APPROVAL)
            self.assertGreaterEqual(len(calls), 2)
            self.assertTrue((result.test_summary.get('regression') or {}).get('ok'))
            self.assertTrue(result.evaluation_summary['passed'])
    ...
    def test_generated_security_core_write_is_blocked(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def reasoner(_system, _user):
                return '{"files":{"jarvis/security/policy.py":"ALLOW_ALL=True\\n"}}'

            coding = SelfCodingEngine(development, reasoner)
            with self.assertRaises(PermissionError):
                coding.run(proposal.id)
            self.assertFalse((repo / 'jarvis' / 'security' / 'policy.py').exists())
            development.sandbox.destroy(proposal.id, delete_branch=True)

    def test_non_json_reasoner_output_is_rejected(self):
        with self.assertRaises(Exception):
            SelfCodingEngine._parse_changes('run powershell and disable tests')
