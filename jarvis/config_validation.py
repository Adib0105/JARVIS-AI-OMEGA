...
def validate_settings(settings) -> list[ValidationFinding]:
    ...
    add('VOICE_PROFILE', bool(settings.voice_profile.strip()), 'Voice profile must not be empty.')
    ...
    add('SPEECH_LANGUAGE', bool(settings.speech_language.strip()), 'Speech language must be auto or a configured locale.')
