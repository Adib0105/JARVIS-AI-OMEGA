...
class MissionStore:
    ...
    def _state_payload(mission: Mission) -> str:
        return json.dumps(redact_value(mission.to_dict()), ensure_ascii=False, default=str)
    ...
    def _save_in_transaction(self, conn: sqlite3.Connection, mission: Mission) -> None:
        ...
        if existing is None:
            ...
            conn.execute(
                ...
                (
                    mission.id, mission.session_id, redact_text(mission.goal), mission.status.value,
        ...
        cursor = conn.execute(
            ...
            (
                mission.session_id, redact_text(mission.goal), mission.status.value,
            ...
            with self._lock, self._connect() as conn:
                ...
                conn.execute(
                    ...
                    (
                        mission.id,
                        event_type,
                        json.dumps(redact_value(payload or {}), ensure_ascii=False, default=str),
                        utc_now(),
                    ),
                )
                conn.commit()
    ...
    def add_event(self, mission_id: str, event_type: str, payload: dict | None = None) -> None:
        with self._lock, self._connect() as conn:
            conn.execute(
                ...
                (mission_id, event_type, json.dumps(redact_value(payload or {}), ensure_ascii=False, default=str), utc_now()),
            )
            conn.commit()
