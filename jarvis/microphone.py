...
def _deps():
    try:
        ...
        raise MicrophoneUnavailable(
            'Optional microphone packages are not available. Run setup_windows.ps1 again to install sounddevice and SpeechRecognition.'
        ) from exc
    return sd, sr
...
def _record_until_silence(stream, max_duration: float, sample_rate: int) -> tuple[bytes, bool, float]:
    ...
    chunks: list[bytes] = []; ambient: list[float] = []
    speech_seen = False; trailing_silence = 0.0; peak_rms = 0.0; threshold = 150.0
    for index in range(max_chunks):
        data, _overflowed = stream.read(chunk_frames)
        raw = bytes(data); chunks.append(raw); rms = _pcm_rms(raw); peak_rms = max(peak_rms, rms)
        ...
        if rms >= threshold:
            speech_seen = True; trailing_silence = 0.0
        elif speech_seen:
...
class WakeWordListener:
    """Background wake loop with natural aliases and a conversational follow-up window."""
    ...
    def __init__(self, on_command: Callable[[str], None], on_state: Callable[[str], None] | None = None, on_error: Callable[[str], None] | None = None, wake_word: str = 'hey jarvis', language: str = 'auto', chunk_seconds: float | None = None, continuous_seconds: float | None = None) -> None:
        ...
        self.on_command = on_command; self.on_state = on_state or (lambda _state: None); self.on_error = on_error or (lambda _message: None)
        ...
        self.wake_aliases = tuple(dict.fromkeys((
            configured,
            'hey jarvis', 'wake up jarvis', 'wake jarvis', 'ok jarvis', 'okay jarvis',
            # Common Indian-English recognizer spellings for the spoken name.
            'hey jervis', 'wake up jervis', 'wake jervis', 'ok jervis', 'okay jervis',
        )))
    ...
    def running(self) -> bool:
        with self._thread_lock:
            thread = self._thread; stop_event = self._stop
    ...
    def start(self) -> None:
        ...
        with self._thread_lock:
            self._stop = stop_event; self._thread = thread; self._conversation_until = 0.0
        thread.start()

    def stop(self, *, wait: bool = True, timeout: float = 0.75) -> bool:
        with self._thread_lock:
            stop_event = self._stop; thread = self._thread; stop_event.set(); self._conversation_until = 0.0
        if wait and thread is not None and thread.is_alive() and threading.current_thread() is not thread:
    ...
    def _command_from_heard(self, heard: str) -> tuple[bool, str]:
        ...
        normalized = _normalize_wake_text(original)
        for alias in sorted(self.wake_aliases, key=len, reverse=True):
            index = normalized.find(alias)
            if index >= 0:
                # Alias matching operates on punctuation-normalized text; commands
                # do not need exact original character offsets for normal speech.
    ...
    def _loop(self, stop_event: threading.Event) -> None:
        ...
        while not stop_event.is_set():
            try:
                ...
                if woke: request_voice_interrupt()
