...
class SelfImprovementBenchmark:
    ...
    def record_after(self, proposal_id: str, results: list[ScenarioResult]) -> ImprovementBenchmarkResult:
        ...
        histories = self.benchmark.history(1000)
        before = next((item for item in histories if item.get('id') == before_id), None)
        if before is None:
            raise RuntimeError('Stored baseline benchmark could not be found.')
        after = self.benchmark.record(f'{proposal_id}:after', results)
        comparison = self.benchmark.compare(before, after.as_dict())
        proposal.evaluation_summary['benchmark_after_id'] = after.id
        proposal.evaluation_summary['benchmark_after'] = after.metrics
        proposal.evaluation_summary['benchmark_comparison'] = comparison
        self.development.store.save(proposal)
        return ImprovementBenchmarkResult(proposal_id, before_id, after.id, comparison)
